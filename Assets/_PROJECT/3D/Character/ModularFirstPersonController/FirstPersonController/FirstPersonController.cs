...
public class FirstPersonController : MonoBehaviour
{
    ...
    public SoundConfig _stepConfig;
    ...
    private void HandleStepSounds()
    {
        if (isWalking || isSprinting)
        {
            ...
            // Determine the step interval based on movement state
            float currentStepInterval = isSprinting ? (_stepInterval / 2) : _stepInterval;

            if (_stepTimer >= currentStepInterval)
            {
                References.Instance.AudioHandler.PlaySound(_stepConfig);
                _stepTimer = 0f;
            }
        }
        else
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
       ...
       // EditorGUILayout.Space();
        ...
        fpc._stepConfig = (SoundConfig)EditorGUILayout.ObjectField(new GUIContent("Step Config", "Configuration for step sounds."), fpc._stepConfig, typeof(SoundConfig), true);
